 ...
 * 
 * 指定戰敗後的CG圖片(僅CG解鎖)
 * enable_picture_dead : <ID>
 * 
 * 指定戰敗後的CG圖片(變數條件)
 * enable_picture_dead : <ID> <變數ID> <變數值> 
 * 例如 當4號變數大於等於6時 enable_picture_dead : 1 4 6
 * 
 * 指定戰敗後的CG圖片(變數條件+成就解鎖)
 * enable_picture_dead : <ID> <變數ID> <變數值> <成就ID> 
 * 例如 當4號變數大於等於5並解鎖指定成就ID EX004時 enable_picture_dead : 1 4 5 EX004
 * 
 * 關閉戰敗後CG
 * enable_picture_dead : false
 *
 * =============================================================================
 * HISTÓRICO
 * =============================================================================
...
Moghunter.picturegallery_msgUnlock = String(Moghunter.parameters['Msg Unlock'] || '\c[6]解鎖了\c[5]「戰敗!」\c[6]圖片!');

Moghunter.picturegallery_save = [];

Moghunter.picturegallery_allDead = false;
Moghunter.picturegallery_varID = null;
Moghunter.picturegallery_varle = 0;
Moghunter.picturegallery_AchID = null;
//=============================================================================
// ** ImageManager
//=============================================================================
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
	...
	if (command === "enable_picture_dead") {
		if (args[1] == "false") {
			Moghunter.picturegallery_allDead = false;
		} else {
			Moghunter.picturegallery_allDead = Number(args[1]);
		}
		if (args[2]) {
			Moghunter.picturegallery_varID = Number(args[2]);
			Moghunter.picturegallery_varle = Number(args[3]);
			Moghunter.picturegallery_AchID = String(args[4]);
		}
...
Scene_Base.prototype.checkGameover = function () {
	if ($gameParty.isAllDead()) {
		if (Moghunter.picturegallery_allDead) {

			Moghunter.picturegallery_save[Moghunter.picturegallery_allDead] = true;

			ConfigManager.save();
		}

		//Moghunter.picturegallery_save[Moghunter.picturegallery_allDead] = false;
...
BattleManager.processDefeat = function () {
	...
	if (Moghunter.picturegallery_allDead && Moghunter.picturegallery_save[Moghunter.picturegallery_allDead - 1] != true) {
		if (Moghunter.picturegallery_varID != null && $gameVariables.value(Moghunter.picturegallery_varID) >= Moghunter.picturegallery_varle - 1) {

			$gameSystem.achievementGet(Moghunter.picturegallery_AchID);


			this.displayPictureDead();
		}

		$gameVariables.setValue(Moghunter.picturegallery_varID, $gameVariables.value(Moghunter.picturegallery_varID) + 1);
		ConfigManager.save();
	}
...
BattleManager.displayPictureDead = function () {
	Moghunter.picturegallery_save[Moghunter.picturegallery_allDead - 1] = true;
	ConfigManager.save();
	$gameMessage.newPage();
	var lines = Moghunter.picturegallery_msgUnlock.replace(/\\/g, '\x1b');
	lines = lines.replace(/\x1b\x1b/g, '\\');
	$gameMessage.add(lines)
};
...
Game_System.prototype.make_picture_list = function () {
	...
	for (var i = 0; i < Moghunter.picturegallery_picture_number; i++) {
		...
		if (Moghunter.picturegallery_save[i] == true) {
			this._picgl_data[i] = [true, String(Moghunter.picturegallery_file_name + (i + 1))]
		} else {
...
Game_System.prototype.enable_picture = function (value, enable) {
	...
	Moghunter.picturegallery_save[pic_id] = enable;
	ConfigManager.save();
};
...
var _alias_mog_ConfigManager_makeData = ConfigManager.makeData;
ConfigManager.makeData = function () {
	let config = _alias_mog_ConfigManager_makeData.call(this);
	config.picDead = Moghunter.picturegallery_allDead;

	if (Moghunter.picturegallery_varID != null) {

		config.deadVar = Moghunter.picturegallery_varID;
		config.varleVar = $gameVariables.value(Moghunter.picturegallery_varID);

	}



	for (let i = 0; i < Moghunter.picturegallery_picture_number; i++) {

		config['gallery' + i] = Moghunter.picturegallery_save[i] == true ? true : false;
	}

	return config;
};

var _alias_mog_ConfigManager_applyData = ConfigManager.applyData;
ConfigManager.applyData = function (config) {
	_alias_mog_ConfigManager_applyData.call(this, config);

	this.picDead = this.readVar(config, 'picDead');
	Moghunter.picturegallery_allDead = this.picDead;

	this.deadVar = this.readVar(config, 'deadVar');
	this.varleVar = this.readVar(config, 'varleVar');

	for (let i = 0; i < Moghunter.picturegallery_picture_number; i++) {
		this['gallery' + i] = this.readFlag(config, 'gallery' + i);
		Moghunter.picturegallery_save[i] = this['gallery' + i];
	}
